function grid_TecPrestadoraSAE_tb_tecsae_id_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_Hidden_tb_tecsae_id" + seqRow).html();
}

function grid_TecPrestadoraSAE_tb_tecsae_id_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_tb_tecsae_id" + seqRow).html(newValue);
}
function grid_TecPrestadoraSAE_tb_tecsae_id_Hidden_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_Hidden_tb_tecsae_id" + seqRow).html(newValue);
}

function grid_TecPrestadoraSAE_tb_tecnicos_id_opepre_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_tb_tecnicos_id_opepre" + seqRow).html();
}

function grid_TecPrestadoraSAE_tb_tecnicos_id_opepre_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_tb_tecnicos_id_opepre" + seqRow).html(newValue);
}

function grid_TecPrestadoraSAE_nom_tecnicocompleto_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nom_tecnicocompleto" + seqRow).html();
}

function grid_TecPrestadoraSAE_nom_tecnicocompleto_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nom_tecnicocompleto" + seqRow).html(newValue);
}

function grid_TecPrestadoraSAE_tb_tecnicos_data_ativacao_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_tb_tecnicos_data_ativacao" + seqRow).html();
}

function grid_TecPrestadoraSAE_tb_tecnicos_data_ativacao_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_tb_tecnicos_data_ativacao" + seqRow).html(newValue);
}

function grid_TecPrestadoraSAE_tb_tecnicos_num_rg_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_tb_tecnicos_num_rg" + seqRow).html();
}

function grid_TecPrestadoraSAE_tb_tecnicos_num_rg_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_tb_tecnicos_num_rg" + seqRow).html(newValue);
}

function grid_TecPrestadoraSAE_tb_tecnicos_num_telefone1_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_tb_tecnicos_num_telefone1" + seqRow).html();
}

function grid_TecPrestadoraSAE_tb_tecnicos_num_telefone1_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_tb_tecnicos_num_telefone1" + seqRow).html(newValue);
}

function grid_TecPrestadoraSAE_remover_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_remover" + seqRow).find("img").attr("src");
}

function grid_TecPrestadoraSAE_remover_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_remover" + seqRow).find("img").attr("src", newValue);
}

function grid_TecPrestadoraSAE_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("tb_tecsae_id" == field) {
    SC_tmp = grid_TecPrestadoraSAE_tb_tecsae_id_getValue(seqRow);
  }
  if ("tb_tecnicos_id_opepre" == field) {
    SC_tmp = grid_TecPrestadoraSAE_tb_tecnicos_id_opepre_getValue(seqRow);
  }
  if ("nom_tecnicocompleto" == field) {
    SC_tmp = grid_TecPrestadoraSAE_nom_tecnicocompleto_getValue(seqRow);
  }
  if ("tb_tecnicos_data_ativacao" == field) {
    SC_tmp = grid_TecPrestadoraSAE_tb_tecnicos_data_ativacao_getValue(seqRow);
  }
  if ("tb_tecnicos_num_rg" == field) {
    SC_tmp = grid_TecPrestadoraSAE_tb_tecnicos_num_rg_getValue(seqRow);
  }
  if ("tb_tecnicos_num_telefone1" == field) {
    SC_tmp = grid_TecPrestadoraSAE_tb_tecnicos_num_telefone1_getValue(seqRow);
  }
  if ("remover" == field) {
    SC_tmp = grid_TecPrestadoraSAE_remover_getValue(seqRow);
  }
...
function grid_TecPrestadoraSAE_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("tb_tecsae_id" == field) {
    grid_TecPrestadoraSAE_tb_tecsae_id_setValue(newValue, seqRow);
  }
  if ("tb_tecnicos_id_opepre" == field) {
    grid_TecPrestadoraSAE_tb_tecnicos_id_opepre_setValue(newValue, seqRow);
  }
  if ("nom_tecnicocompleto" == field) {
    grid_TecPrestadoraSAE_nom_tecnicocompleto_setValue(newValue, seqRow);
  }
  if ("tb_tecnicos_data_ativacao" == field) {
    grid_TecPrestadoraSAE_tb_tecnicos_data_ativacao_setValue(newValue, seqRow);
  }
  if ("tb_tecnicos_num_rg" == field) {
    grid_TecPrestadoraSAE_tb_tecnicos_num_rg_setValue(newValue, seqRow);
  }
  if ("tb_tecnicos_num_telefone1" == field) {
    grid_TecPrestadoraSAE_tb_tecnicos_num_telefone1_setValue(newValue, seqRow);
  }
  if ("remover" == field) {
    grid_TecPrestadoraSAE_remover_setValue(newValue, seqRow);
  }
  if ("tb_tecsae_id_Hidden" == field) {
    grid_TecPrestadoraSAE_tb_tecsae_id_Hidden_setValue(newValue, seqRow);
  }
}
...
function scJQAddEvents(seqRow) {
  ...
  $("#id_sc_field_remover" + seqRow).click(function () {
    $.ajax({
      type: "POST",
      url: "index.php",
      data: "nmgp_opcao=ajax_event&nmgp_event=remover_onClick&hti_cnsdata_init=" + document.F3.hti_cnsdata_init.value + "&hti_cnsdata_session=" + document.F3.hti_cnsdata_session.value + "&tb_tecsae_id=" + grid_TecPrestadoraSAE_getValue("tb_tecsae_id", seqRow) + "&remover=" + grid_TecPrestadoraSAE_getValue("remover", seqRow) + ""
     })
     .done(function(jsonReturn) {
        ...
        if (oResp["setValue"]) {
          for (i = 0; i < oResp["setValue"].length; i++) {
            grid_TecPrestadoraSAE_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
          }
        }
