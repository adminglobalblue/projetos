function grid_Relatoriogerencial_edit_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_edit" + seqRow).find("img").attr("src");
}

function grid_Relatoriogerencial_edit_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_edit" + seqRow).find("img").attr("src", newValue);
}

function grid_Relatoriogerencial_id_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_Hidden_id" + seqRow).html();
}

function grid_Relatoriogerencial_id_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_id" + seqRow).html(newValue);
}
function grid_Relatoriogerencial_id_Hidden_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_Hidden_id" + seqRow).html(newValue);
}

function grid_Relatoriogerencial_nom_empreendimento_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_nom_empreendimento" + seqRow).html();
}

function grid_Relatoriogerencial_nom_empreendimento_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_nom_empreendimento" + seqRow).html(newValue);
}

function grid_Relatoriogerencial_criado_em_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_criado_em" + seqRow).html();
}

function grid_Relatoriogerencial_criado_em_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_criado_em" + seqRow).html(newValue);
}

function grid_Relatoriogerencial_datainiciovigencia_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_datainiciovigencia" + seqRow).html();
}

function grid_Relatoriogerencial_datainiciovigencia_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_datainiciovigencia" + seqRow).html(newValue);
}

function grid_Relatoriogerencial_datafimvigencia_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_datafimvigencia" + seqRow).html();
}

function grid_Relatoriogerencial_datafimvigencia_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_datafimvigencia" + seqRow).html(newValue);
}

function grid_Relatoriogerencial_num_status_getValue(seqRow) {
  seqRow = scSeqNormalize(seqRow);
  return $("#id_sc_field_num_status" + seqRow).html();
}

function grid_Relatoriogerencial_num_status_setValue(newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  $("#id_sc_field_num_status" + seqRow).html(newValue);
}

function grid_Relatoriogerencial_getValue(field, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  var SC_tmp = "";
  if ("edit" == field) {
    SC_tmp = grid_Relatoriogerencial_edit_getValue(seqRow);
  }
  if ("id" == field) {
    SC_tmp = grid_Relatoriogerencial_id_getValue(seqRow);
  }
  if ("nom_empreendimento" == field) {
    SC_tmp = grid_Relatoriogerencial_nom_empreendimento_getValue(seqRow);
  }
  if ("criado_em" == field) {
    SC_tmp = grid_Relatoriogerencial_criado_em_getValue(seqRow);
  }
  if ("datainiciovigencia" == field) {
    SC_tmp = grid_Relatoriogerencial_datainiciovigencia_getValue(seqRow);
  }
  if ("datafimvigencia" == field) {
    SC_tmp = grid_Relatoriogerencial_datafimvigencia_getValue(seqRow);
  }
  if ("num_status" == field) {
    SC_tmp = grid_Relatoriogerencial_num_status_getValue(seqRow);
  }
...
function grid_Relatoriogerencial_setValue(field, newValue, seqRow) {
  seqRow = scSeqNormalize(seqRow);
  if ("edit" == field) {
    grid_Relatoriogerencial_edit_setValue(newValue, seqRow);
  }
  if ("id" == field) {
    grid_Relatoriogerencial_id_setValue(newValue, seqRow);
  }
  if ("nom_empreendimento" == field) {
    grid_Relatoriogerencial_nom_empreendimento_setValue(newValue, seqRow);
  }
  if ("criado_em" == field) {
    grid_Relatoriogerencial_criado_em_setValue(newValue, seqRow);
  }
  if ("datainiciovigencia" == field) {
    grid_Relatoriogerencial_datainiciovigencia_setValue(newValue, seqRow);
  }
  if ("datafimvigencia" == field) {
    grid_Relatoriogerencial_datafimvigencia_setValue(newValue, seqRow);
  }
  if ("num_status" == field) {
    grid_Relatoriogerencial_num_status_setValue(newValue, seqRow);
  }
  if ("id_Hidden" == field) {
    grid_Relatoriogerencial_id_Hidden_setValue(newValue, seqRow);
  }
}
...
function scJQAddEvents(seqRow) {
  ...
  $("#id_sc_field_edit" + seqRow).click(function () {
    $.ajax({
      type: "POST",
      url: "index.php",
      data: "nmgp_opcao=ajax_event&nmgp_event=edit_onClick&hti_cnsdata_init=" + document.F3.hti_cnsdata_init.value + "&hti_cnsdata_session=" + document.F3.hti_cnsdata_session.value + "&id=" + grid_Relatoriogerencial_getValue("id", seqRow) + "&edit=" + grid_Relatoriogerencial_getValue("edit", seqRow) + ""
     })
     .done(function(jsonReturn) {
        ...
        if (oResp["setValue"]) {
          for (i = 0; i < oResp["setValue"].length; i++) {
            grid_Relatoriogerencial_setValue(oResp["setValue"][i]["field"], oResp["setValue"][i]["value"], seqRow);
          }
        }
