...
function nmButtonOutput($arr_buttons, $sBtn, $sClick, $sHref, $sId, $sName, $sValue, $sStyle, $sAlign, $sKey, $sBorder, $spath, $sAlt, $sHint, $sClassJ, $AltJ, $sGrupo = '', $str_display = 'only_text', $str_display_position = 'text_right', $bol_link = false, $str_target = '', $sonMouseOver='', $sonMouseOut='', $menu_em_aba = false, $nome_aplicacao = '')
{
    ...
    if (isset($_SESSION['hticnsdata']['charset']))
    {
      if (!function_exists("sc_convert_encoding"))
      {
        if ($_SESSION['hticnsdata']['charset'] != "UTF-8" && NM_is_utf8($sValue))
        {
            $sValue = mb_convert_encoding($sValue, $_SESSION['hticnsdata']['charset'], "UTF-8");
        }
        if ($_SESSION['hticnsdata']['charset'] != "UTF-8" && NM_is_utf8($sHint))
        {
            $sHint = mb_convert_encoding($sHint, $_SESSION['hticnsdata']['charset'], "UTF-8");
        }
        if ($_SESSION['hticnsdata']['charset'] != "UTF-8" && NM_is_utf8($sAlt))
        {
            $sAlt = mb_convert_encoding($sAlt, $_SESSION['hticnsdata']['charset'], "UTF-8");
        }
        if ($_SESSION['hticnsdata']['charset'] != "UTF-8" && NM_is_utf8($AltJ))
        {
            $AltJ = mb_convert_encoding($AltJ, $_SESSION['hticnsdata']['charset'], "UTF-8");
        }
      }
      else
      {
        if ($_SESSION['hticnsdata']['charset'] != "UTF-8" && NM_is_utf8($sValue))
        {
            $sValue = sc_convert_encoding($sValue, $_SESSION['hticnsdata']['charset'], "UTF-8");
        }
        if ($_SESSION['hticnsdata']['charset'] != "UTF-8" && NM_is_utf8($sHint))
        {
            $sHint = sc_convert_encoding($sHint, $_SESSION['hticnsdata']['charset'], "UTF-8");
        }
        if ($_SESSION['hticnsdata']['charset'] != "UTF-8" && NM_is_utf8($sAlt))
        {
            $sAlt = sc_convert_encoding($sAlt, $_SESSION['hticnsdata']['charset'], "UTF-8");
        }
        if ($_SESSION['hticnsdata']['charset'] != "UTF-8" && NM_is_utf8($AltJ))
        {
            $AltJ = sc_convert_encoding($AltJ, $_SESSION['hticnsdata']['charset'], "UTF-8");
        }
      }
    }
        ...
        if ('button' == $arr_buttons[$sBtn]['type'])
    {
                ...
                {
                                                $sCodigo .= "&nbsp;&nbsp;<img src=\"" . $spath . "/hticnsdata__NM__ico__NM__group_expand.png\" align='absmiddle' border='0'>\r\n";
                }
