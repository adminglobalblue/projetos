...
class UploadHandler
{
    ...
    private function handle_file_upload($uploaded_file, $name, $size, $type, $error) {
        ...
        {
            $sUploadFileNameProt = mb_convert_encoding($sUploadFileNameProt, $_SESSION['hticnsdata']['charset'], 'UTF-8');
            $sUploadFileNameOrig = mb_convert_encoding($sUploadFileNameOrig, $_SESSION['hticnsdata']['charset'], 'UTF-8');
        }
        ...
        if (!$error && $file->name) {
            ...
            */
            ...
            if (isset($_SESSION['hticnsdata']['charset']) && 'UTF-8' != $_SESSION['hticnsdata']['charset'])
            {
                $file->sc_random_prot = mb_convert_encoding($file->sc_random_prot, 'UTF-8', $_SESSION['hticnsdata']['charset']);
                $file->sc_random      = mb_convert_encoding($file->sc_random,      'UTF-8', $_SESSION['hticnsdata']['charset']);
                $file->sc_thumb_prot  = mb_convert_encoding($file->sc_thumb_prot,  'UTF-8', $_SESSION['hticnsdata']['charset']);
                $file->sc_thumb       = mb_convert_encoding($file->sc_thumb,       'UTF-8', $_SESSION['hticnsdata']['charset']);
            }
            global $aUploadInfo;
