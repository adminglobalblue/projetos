<?php
    ...
    {
        $charset = (isset($_SESSION['hticnsdata']['charset_entities'][$_SESSION['hticnsdata']['charset']])) ? $_SESSION['hticnsdata']['charset_entities'][$_SESSION['hticnsdata']['charset']] : $_SESSION['hticnsdata']['charset'];
        ...
        {
            $str = sc_convert_encoding($sString, 'UTF-8', $_SESSION['hticnsdata']['charset']);
            $str = htmlentities($str, null, 'UTF-8');
            $str = sc_convert_encoding($str, $_SESSION['hticnsdata']['charset'], 'UTF-8');
            if (!empty($str))
            {
                return $str;
            }
    ...
    {
        global $SC_arr_charset_iconv;
        if ($_SESSION['hticnsdata']['charset'] == 'WINDOWS-1250')
        {
        ...
        elseif (mb_check_encoding($sString, $_SESSION['hticnsdata']['charset']))
        {
            $str = mb_strtoupper($sString, $_SESSION['hticnsdata']['charset']);
            return $str;
        }
        else
        {
    ...
    function sc_strtolower($sString)
    {
        ...
        elseif (mb_check_encoding($sString, $_SESSION['hticnsdata']['charset']))
        {
            $str = mb_strtolower($sString, $_SESSION['hticnsdata']['charset']);
            return $str;
        }
        else
        {
    ...
    {
        global $SC_arr_charset_iconv;
        if (mb_check_encoding($sString, $_SESSION['hticnsdata']['charset']))
        {
            $str = mb_substr($sString, $start, $len, $_SESSION['hticnsdata']['charset']);
            return $str;
        }
        else
        {
    ...
    function NM_utf8_urldecode($str)
    {
        ...
        if (isset($_SESSION['hticnsdata']['charset']) && 'UTF-8' == $_SESSION['hticnsdata']['charset'])
        {
            $str = sc_convert_encoding($str, 'UTF-8', $_SESSION['hticnsdata']['charset']);
        }
        elseif (isset($_SESSION['hticnsdata']['charset']) && in_array($_SESSION['hticnsdata']['charset'], $SC_arr_charset_sajax) && function_exists('mb_detect_encoding') && 'UTF-8' == mb_detect_encoding($str))
        {
            $str = sc_convert_encoding($str, $_SESSION['hticnsdata']['charset'], 'UTF-8');
        }
        ...
        $str = str_replace('__SC_PLUS__', '+', $str);
/*
        if (isset($_SESSION['hticnsdata']['charset']) && 'BIG-5' == $_SESSION['hticnsdata']['charset'])
        {
            $str = @html_entity_decode($str, null, 'BIG5');
        }
        else
        {
            $str = @html_entity_decode($str, null, $_SESSION['hticnsdata']['charset']);
        }
*/
    ...
    {
        if ('UTF-8' != $_SESSION['hticnsdata']['charset'])
        {
            $str = sc_convert_encoding($str, 'UTF-8', $_SESSION['hticnsdata']['charset']);
        }
        return $str;
   ...
   function NM_protect_string($sString)
   {
      ...
      if (!empty($sString))
      {
         ...
         {
/*             return htmlentities($sString, ENT_COMPAT, $_SESSION['hticnsdata']['charset']); */
